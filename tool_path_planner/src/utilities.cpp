...
{
  ToolPathSegment output;
  output.reserve(input_poses.size());
  for (std::size_t i = 0; i < input_poses.size(); ++i)
  {
    Eigen::Isometry3d pose;
    pose.translation() = input_poses[i].translation();
    pose.linear() = getMovingAverageQuaternion(input_poses, static_cast<int>(i), n_pts).toRotationMatrix();
