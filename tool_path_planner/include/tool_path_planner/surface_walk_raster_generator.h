...
struct ProcessPath
{
  vtkSmartPointer<vtkPolyData> line;  // sequence of points and a normal defining the locations and z-axis orientation
                                      // of the tool along the path
  vtkSmartPointer<vtkParametricSpline> spline;  // spline used to generate the line lamda goes from 0 to 1 as the line
  ...
  vtkSmartPointer<vtkPolyData> intersection_plane;  // May belong here, ok to return empty{}, used by the
                                                    // raster_tool_path_planner and returned for display
};
...
public:
  struct Config
  {
    ...
    bool fromJson(const Json::Value& jv)
    {
      ...
      intersection_plane_height = validate(jv, "intersection_plane_height", Json::ValueType::realValue) ?
                                      jv["intersection_plane_height"].asDouble() :
                                      DEFAULT_INTERSECTION_PLANE_HEIGHT;
      tool_offset =
          validate(jv, "tool_offset", Json::ValueType::realValue) ? jv["tool_offset"].asDouble() : DEFAULT_TOOL_OFFSET;
      generate_extra_rasters = validate(jv, "generate_extra_rasters", Json::ValueType::booleanValue) ?
...
protected:
   ...
   */
  bool getNextPath(const ProcessPath this_path,
  ...
  /**
   * @brief Estimates the normals of a line that lies on the surface of the current mesh.  For each point, it uses the
   * normal of the closes cell in the mesh
   ...
   */
  void smoothData(vtkSmartPointer<vtkParametricSpline> spline,
   ...
   */
  vtkSmartPointer<vtkPolyData> createOffsetLine(vtkSmartPointer<vtkPolyData> line,
                                                vtkSmartPointer<vtkPolyData> derivatives,
                                                double dist);

  /**
   * @brief createSurfaceFromSpline Using a line with normals, generate a surface with points above and below the line
   * (used for mesh intersection calculation)
  ...
  /**
   * @brief getConnectedIntersectionLine Given a series of lines, returns a single continuous line while filtering out
   * small segments
  ...
  /**
   * @brief getConnectedIntersectionLine Given an intersection line data and a start location, finds and returns a list
   ...
   * @param start_pt Optional: the point id in the line to start from (useful when performing this operation multiple
   * times)
   * @return the length of the line segment returned
   */
  double getConnectedIntersectionLine(vtkSmartPointer<vtkPolyData> line,
                                      vtkSmartPointer<vtkPoints>& points,
                                      std::vector<int>& used_ids,
                                      int start_pt = 0);

  /**
   * @brief checkPathForHoles Checks a given path to determine if it needs to be broken up if there is a large hole in
   * the middle
  ...
  /**
   * @brief resamplePoints Resamples a set of points to make them evenly spaced, creates and samples a spline through
