...
ToolPaths RasterOrganizationModifier::modify(ToolPaths tool_paths) const
{
  ...
  for (ToolPath& tool_path : tool_paths)
  {
    ...
    {
      std::sort(
          segment.begin(), segment.end(), [&segment, &ref_dir](const ToolPathWaypoint& a, const ToolPathWaypoint& b) {
    ...
    // direction
    std::sort(
        tool_path.begin(), tool_path.end(), [&tool_path, &ref_dir](const ToolPathSegment& a, const ToolPathSegment& b) {
