...
namespace noether
{
PlaneSlicerRasterPlannerWidget::PlaneSlicerRasterPlannerWidget(plugin_loader::PluginLoader&& loader, QWidget* parent)
  : RasterPlannerWidget(std::move(loader), parent)
  , search_radius_(new QDoubleSpinBox(this))
  , min_segment_length_(new QDoubleSpinBox(this))
{
  ...
  // Min segment length
  min_segment_length_->setMinimum(0.0);
  min_segment_length_->setSingleStep(0.1);
  min_segment_length_->setValue(0.1);
  min_segment_length_->setDecimals(3);
  layout->addRow(new QLabel("Min. segment length", this), min_segment_length_);
...
{
  auto dir_gen_widget = findFirstWidgetOfType<DirectionGeneratorWidget>(ui_->widget_dir_gen);
  auto origin_gen_widget = findFirstWidgetOfType<OriginGeneratorWidget>(ui_->widget_origin_gen);
  ...
  planner->setLineSpacing(min_segment_length_->value());
