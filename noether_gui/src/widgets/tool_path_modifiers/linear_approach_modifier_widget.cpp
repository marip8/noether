...
void LinearApproachToolPathModifierWidget::configure(const YAML::Node& config)
{
  ...
  vector_editor_ui_->double_spin_box_x->setValue(offset(0));
  vector_editor_ui_->double_spin_box_y->setValue(offset(1));
  vector_editor_ui_->double_spin_box_z->setValue(offset(2));
}
...
void LinearApproachToolPathModifierWidget::save(YAML::Node& config) const
{
  ...
  {
    vec(0) = vector_editor_ui_->double_spin_box_x->value();
    vec(1) = vector_editor_ui_->double_spin_box_y->value();
    vec(2) = vector_editor_ui_->double_spin_box_z->value();
  }
